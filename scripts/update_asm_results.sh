...
# Aggressive size flags: optimize for size, remove unused sections,
# omit frame pointers, strip canaries and unwind info, no PIC, no builtins,
# align functions and basic blocks to 1 byte, disable vectorization/unrolling.
...
ASM_OPT=""  # no extra flags; SIZE_CFLAGS already includes -Oz
...
build() {
  ...
  if [ "$tgt" = wasm32 ] || [ "$tgt" = wasm64 ]; then
    $WASM_LD --allow-undefined --no-entry --gc-sections --strip-all --stack-first \
    ...
    rm -f $base.tmp.wasm $base.tmp2.wasm
...
for fn in $FUNCTIONS; do
  ...
  for tgt in $TARGETS; do
    ...
    if [ "$tgt" = "wasm64" ]; then
      ...
      tmp_obj=$(mktemp /tmp/wasm64XXXXXX.o)
      if ! printf 'int f(void){return 0;}\n' | $CLANG $target $ASM_CFLAGS -xc - -c -o "$tmp_obj" >/dev/null 2>&1; then
        rm -f "$tmp_obj"
        continue
      fi
      if ! $WASM_LD --allow-undefined --no-entry --gc-sections --strip-all --stack-first \
        --initial-memory=65536 --max-memory=65536 --export=f "$tmp_obj" -o "$tmp_obj.wasm" >/dev/null 2>&1; then
        rm -f "$tmp_obj" "$tmp_obj.wasm"
        continue
      fi
      rm -f "$tmp_obj" "$tmp_obj.wasm"
    fi
...
rm -f *.o *.s *.bin *.tmp.wasm *.tmp2.wasm
